...
Beyond the bespoke Poseidon3D pipeline, two off-the-shelf point-cloud methods are part of the evaluation toolkit and were run on **real Teeth3DS+ upper arches** (N=40; BUFFER-X keep-0.5/keep-0.3 over **3 crop-seed reps** so the numbers carry error bars) at the identical whole-tooth-dropout crop protocol — one a decisive positive, one an honest negative:

![Partial-overlap identity on real dental arches: BUFFER-X zero-shot registration transfers (Rank-1 1.00 / 1.00 / 0.95 at keep 1.0 / 0.5 / 0.3, min–max whiskers over 3 crop-seed reps) while a frozen Sonata foundation-model head does not (0.28 / 0.13 / 0.03), against recorded CorrNet, rigid-GICP and DGCNN-embedding references](docs/partial_overlap_results.png)
...
| **BUFFER-X zero-shot** (Teeth3DS+ · measured) | **1.00** | **1.00** | **0.95** (0.90–0.98) |
| Sonata frozen head (Teeth3DS+ · measured) | 0.28 | 0.13 | 0.03 |

BUFFER-X keep-0.5/keep-0.3 are the mean over 3 crop-seed reps: keep-0.5 held **1.00 in all 3 reps**, keep-0.3 was **0.95 ± 0.04 (min–max 0.90–0.98)** — the headline is stable, now with error bars (whiskers in the chart). Its keep-1.0 cell is a **measured** full-coverage number, not a placeholder: the same BUFFER-X backend on the full-coverage identity protocol (the one the GICP smoke uses) scores **Rank-1 1.000 · Rank-5 1.000 · EER 0.000 · AUC 1.000** (N=40, `bufferx_identity_full.json`).

*Cross-dataset by construction:* BUFFER-X and Sonata are measured on **Teeth3DS+**; the recorded CorrNet / GICP / DGCNN rows are on **Poseidon3D** (not on this machine). The data is **single-timepoint** with synthetic genuine re-scans — still short of the longitudinal [gate #7](#the-real-data-gate-gate-7). Blank cells are honestly missing, not zero.

**Zero-shot registration transfers to teeth — the positive.** [**BUFFER-X**](https://github.com/MIT-SPARK/BUFFER-X) (Kim et al., ICCV 2025) is a *generalist* point-cloud registrar pretrained on indoor RGB-D scans with **no dental training**. On the real arches it registers dental micro-geometry cleanly: realistic whole-tooth-dropout Rank-1 **1.00 @ keep-0.5** (all 3 reps) and **0.95 @ keep-0.3** (min–max 0.90–0.98 over 3 crop-seed reps; clean planar cut 1.00 / 1.00), and a complete full-coverage column (Rank-1/Rank-5/EER/AUC all 1.000). *Read honestly:* BUFFER-X gets a denser point budget as an indoor-scan method, and the genuine query is a crop of the same arch; the repo's own rigid GICP also reaches full-coverage Rank-1 1.0 on Teeth3DS+. So **rigid + zero-shot registration is already strong on real dental arches** — CorrNet's contribution is on the harder Poseidon3D distribution and as a *certifiable, retrieve-then-verify* component, not a claim that no generalist can register teeth. **BUFFER-X is offered as an optional zero-shot registrar for the partial-overlap regime — it does not change the certified pipeline's defaults** (PCA-init + Generalized-ICP for registration, CorrNet for the learned partial-overlap descriptor, conformal accept/abstain on top). (`eval_bufferx_baseline.py` → `bufferx_baseline.json`; full-coverage `eval_bufferx_identity_full.py` → `bufferx_identity_full.json`)
...
**The registration identity pipeline reproduces on real arches, too.** Run on real Teeth3DS+ upper arches, the PCA-init + multi-scale Generalized-ICP pipeline scores **Rank-1 1.000 · EER 0.000 · AUC 1.000 · d′ 6.07** (N=40, freshly re-downloaded md5-verified OSF data; `teeth3ds_identity_smoke_n40.json`). Swapping the backend to BUFFER-X on the *same* full-coverage protocol and 40 subjects gives the same complete column — **Rank-1 1.000 · Rank-5 1.000 · EER 0.000 · AUC 1.000** (`bufferx_identity_full.json`; its lower d′ 1.24 only reflects the different residual scale of the two backends — AUC 1.0 / EER 0 confirm every genuine still outranks every impostor). Both are real arches, but **single-timepoint** with synthetic genuine re-scans, so still not the longitudinal [gate #7](#the-real-data-gate-gate-7).
