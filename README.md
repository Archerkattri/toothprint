...
**Dental work as an explicit forensic identifier.** In forensic odontology the *pattern of restorations* (fillings, crowns, implants) is one of the strongest identifiers — and CBCT makes it directly observable, since metal/ceramic restorations are far denser than enamel (HU > 2500). Extracting each person's high-density "dental-work cloud" and matching it as a biometric: on the real CBCT set the restoration pattern **alone** identifies people at **Rank-1 0.90 / AUC 0.996** — a strong, *independent* signal beyond tooth geometry, exactly the cue a forensic odontologist reads from a dental chart. (On these 20 patients crown geometry already saturates at 1.0, so the measurable fusion gain needs the full 300-patient set; the dental-work signal itself is real and strong.) `evaluation/scripts/eval_dentalwork.py`, `dentalwork_identity.json`.
