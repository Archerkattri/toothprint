...
def sample(path, n):
    ...
    return (
        np.asarray(m.sample_points_uniformly(number_of_points=n).points)
        if len(m.triangles)
        else None
    )
...
def metrics(M, labels):
    ...
    auc = (
        float(np.mean(gen[:, None] < imp[None, :]))
    ...
    dprime = (
        float(abs(gen.mean() - impc.mean()) / np.sqrt((gen.var() + impc.var()) / 2))
        if gen.size
        else 0.0
    )
    return {
        ...
        "genuine_max": float(gen.max()),
        "impostor_mean": float(impc.mean()),
        "impostor_min": float(imp.min()),
        "genuine": gen.tolist(),
        "impostor_round": [round(float(x), 4) for x in impc[:2000]],
    }
...
def main():
    ...
    res = {
        "dataset": "poseidon3d",
        "n_meshes_total": len(meshes),
        "method": "PCA-init + GICP, point-to-point",
        "ablations": {},
    }
    t0 = time.time()
    ...
    if mat_path.exists():
        ...
        M, labels = build_matrix(
            meshes[:N_MAIN], NOISE, ROT_AMP, KEEP, VX, NPTS, workers
        )
        res["main_source"] = "recomputed"
    res["main"] = metrics(M, labels)
    print(
        f"  rank1={res['main']['rank1']:.3f} eer={res['main']['eer']:.3f} auc={res['main']['auc']:.3f} "
        f"d'={res['main']['dprime']:.2f} ({time.time() - t0:.0f}s)",
        flush=True,
    )

    abl = meshes[:N_ABL]
    for noise in [0.0, 0.1, 0.2, 0.4]:
        M, labels = build_matrix(abl, noise, ROT_AMP, KEEP, VX, NPTS, workers)
        res["ablations"][f"noise_{noise}"] = metrics(M, labels)
        print(
            f"  noise {noise}mm: rank1={res['ablations'][f'noise_{noise}']['rank1']:.3f}",
            flush=True,
        )
    for keep in [1.0, 0.5, 0.3, 0.2]:
        M, labels = build_matrix(abl, NOISE, ROT_AMP, keep, VX, NPTS, workers)
        res["ablations"][f"keep_{keep}"] = metrics(M, labels)
        print(
            f"  tooth-coverage {keep}: rank1={res['ablations'][f'keep_{keep}']['rank1']:.3f}",
            flush=True,
        )
    for vx in [0.3, 0.8, 1.2]:
        M, labels = build_matrix(abl, NOISE, ROT_AMP, KEEP, vx, NPTS, workers)
        res["ablations"][f"voxel_{vx}"] = metrics(M, labels)
        print(
            f"  voxel {vx}mm: rank1={res['ablations'][f'voxel_{vx}']['rank1']:.3f}",
            flush=True,
        )
