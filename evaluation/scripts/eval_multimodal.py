...
def ios_cloud(pid):
    m = o3d.io.read_triangle_mesh(str(DATA / pid / f"{pid}_ios" / f"{pid}_UpperJawScan.stl"))
    return norm(np.asarray(m.sample_points_uniformly(N).points)) if len(m.triangles) else None


def cbct_cloud(pid, rng):
    a = nib.load(str(DATA / pid / f"{pid}_cbct" / f"{pid}_cbct.nii.gz")).get_fdata()
    idx = np.argwhere(a > a.mean() + a.std()).astype(np.float32)        # bone + teeth surface points
...
def matrix(gallery, queries):
    n = len(gallery)
    return np.array([[align_rigid(queries[i], gallery[j], 0.5)[1] for j in range(n)] for i in range(n)])
...
def metrics(M):
    ...
    return {"rank1": r1, "auc": auc, "gen_mean": float(gen.mean()), "imp_min": float(imp.min())}
...
def main():
    ...
    for pid in pids:
        ic = ios_cloud(pid); cc = cbct_cloud(pid, np.random.default_rng(7))
        if ic is None or cc is None:
            continue
        kept.append(pid)
        ios_g.append(ic); cbct_g.append(cc)
    ...
    res = {"n_patients": len(kept), "dataset": "Figshare CBCT+oral-scan (real paired)",
           "ios_crowns": metrics(A), "cbct_bone": metrics(B), "fused": metrics(F)}
    OUT.write_text(json.dumps(res, indent=1) + "\n")
    for k in ("ios_crowns", "cbct_bone", "fused"):
        print(f"  {k:12s} Rank-1 {res[k]['rank1']:.3f}  AUC {res[k]['auc']:.3f}", flush=True)
    print(f"  -> fusion {'BEATS' if res['fused']['rank1'] >= max(res['ios_crowns']['rank1'], res['cbct_bone']['rank1']) else 'ties/loses'} the best single modality")
