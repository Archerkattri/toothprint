...
def constellation(ann):
    pts = [
        [float(p[0]), float(p[1])]
        for t in ann.get("teeth", [])
        for fld in ("cej", "crest_line", "apex")
        for p in (t.get(fld) or [])
    ]
...
def _norm(c):
    c = c - c.mean(0)
    r = np.sqrt((c**2).sum(1).mean())
    return c / (r or 1), (r or 1)
...
def load_gallery(min_teeth, cap):
    ...
    for rec in RealDenparAdapter(DATA).records("test"):
        teeth = [
            t
            for t in rec.annotation_dict.get("teeth", [])
...
def evaluate(gal, labs, mag, jitter, seed=0):
    ...
    for i, c in enumerate(gal):
        ...
        q = (
            (1 + rng.uniform(-mag, mag)) * c @ R.T
            + rng.uniform(-20, 20, 2)
            + rng.normal(0, jitter, c.shape)
        )
    ...
    if gen.size == 0 or imp.size == 0:
        return {
            "n": n,
            "rank1": r1 / n if n else float("nan"),
            "rank5": r5 / n if n else float("nan"),
            "auc": float("nan"),
            "eer": float("nan"),
            "dprime": float("nan"),
            "genuine_mean": float("nan"),
            "genuine_max": float("nan"),
            "impostor_mean": float("nan"),
            "impostor_min": float("nan"),
        }
...
def main():
    ...
    print(
        f"  rank1={res['main']['rank1']:.3f} d'={res['main']['dprime']:.2f} eer={res['main']['eer']:.3f}",
        flush=True,
    )
    ...
    for jit in [0.0, 6.0, 12.0, 20.0]:
        ...
        print(
            f"  jitter {jit}px: rank1={res['ablations'][f'jitter_{jit}']['rank1']:.3f}",
            flush=True,
        )
    for mag in [0.0, 0.15, 0.3, 0.5]:
        res["ablations"][f"mag_{mag}"] = evaluate(gal, labs, mag=mag, jitter=3.0)
        print(
            f"  magnification {mag}: rank1={res['ablations'][f'mag_{mag}']['rank1']:.3f}",
            flush=True,
        )
    for mt in [3, 6, 8]:
        g, labels = load_gallery(min_teeth=mt, cap=120)
        res["ablations"][f"minteeth_{mt}"] = evaluate(g, labels, mag=0.08, jitter=3.0)
        print(
            f"  min-teeth {mt} (N={len(g)}): rank1={res['ablations'][f'minteeth_{mt}']['rank1']:.3f}",
            flush=True,
        )
