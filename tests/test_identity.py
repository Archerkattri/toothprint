...
from toothprint.identity.constellation import (
    ...
    constellation,
    icp_residual,
    identify as identify_radiograph,
)
...
def _subject(seed=0, n=2000):
    ...
    bump = 1 + 0.22 * np.sin(rng.uniform(1, 5) * v[:, 0]) * np.cos(
        rng.uniform(1, 5) * v[:, 1]
    )
...
def test_score_to_surface_genuine_sub_noise_impostor_high():
    ...
    gen.vertices = gen.vertices * np.array(
        [1.0, 0.7, 0.5]
    )  # anisotropic -> stable axes
    ...
    d_gen = score_to_surface(
        q, np.asarray(gen.vertices), np.asarray(gen.faces), voxel_size=1.0
    )
    d_imp = score_to_surface(
        q, np.asarray(imp.vertices), np.asarray(imp.faces), voxel_size=1.0
    )
