...
def test_identify_radiograph_picks_genuine():
    ...
    query = (
        1.1 * b @ R.T + np.array([8.0, -4.0]) + rng.normal(0, 0.5, b.shape)
    ).tolist()
    r = client.post(
        "/api/identify/radiograph",
        json={
            "query": query,
            "gallery": [
                {"label": "subject_A", "points": base},
...
def test_certify_change_labels():
    r = client.post(
        "/api/certify/change",
        json={"measured_px": 30.0, "q_lo": 0.5, "q_hi": 0.5, "tau": 6.0},
    )
    assert r.json()["label"] == "changed"
    r2 = client.post(
        "/api/certify/change",
        json={"measured_px": 0.1, "q_lo": 0.5, "q_hi": 0.5, "tau": 6.0},
    )
...
def test_certify_surface_labels():
    r = client.post(
        "/api/certify/surface", json={"measured_mm": 1.2, "q_lo": 0.1, "q_hi": 0.1}
    )
    assert r.json()["label"] == "changed"
    r2 = client.post(
        "/api/certify/surface", json={"measured_mm": 0.05, "q_lo": 0.1, "q_hi": 0.1}
    )
    assert r2.json()["label"] == "stable"
...
def test_reject_empty_gallery():
    r = client.post(
        "/api/identify/radiograph", json={"query": [[0.0, 0.0]], "gallery": []}
    )
    assert r.status_code == 422
...
def test_reject_alpha_out_of_range():
    for a in (0.0, 1.0, -0.1, 1.5):
        r = client.post(
            "/api/certify/change",
            json={"measured_px": 5.0, "q_lo": 0.5, "q_hi": 0.5, "alpha": a},
        )
        assert r.status_code == 422
...
def test_reject_inverted_quantiles():
    r = client.post(
        "/api/certify/surface", json={"measured_mm": 1.0, "q_lo": 0.9, "q_hi": 0.1}
    )
    assert r.status_code == 422
    r2 = client.post(
        "/api/certify/change", json={"measured_px": 5.0, "q_lo": 0.9, "q_hi": 0.1}
    )
...
def test_inspect_volume(tmp_path):
    ...
    with open(p, "rb") as fh:
        r = client.post(
            "/api/inspect", files={"file": ("v.nii.gz", fh, "application/octet-stream")}
        )
...
def test_oversize_request_rejected():
    ...
    r = client.post(
        "/api/identify/radiograph",
        json={"query": huge, "gallery": [{"label": "a", "points": [[0.0, 0.0]]}]},
    )
    assert r.status_code == 422
...
def test_inspect_scan(tmp_path):
    ...
    with open(p, "rb") as fh:
        r = client.post(
            "/api/inspect", files={"file": ("m.stl", fh, "application/octet-stream")}
        )
...
def test_inspect_radiograph(tmp_path):
    ...
    p = tmp_path / "x.png"
    Image.fromarray(
        (np.random.default_rng(0).random((40, 48)) * 255).astype(np.uint8)
    ).save(str(p))
    with open(p, "rb") as fh:
...
def test_inspect_rejects_garbage(tmp_path):
    p = tmp_path / "j.xyz"
    p.write_bytes(b"\x00\x01 not a medical file")
    with open(p, "rb") as fh:
        r = client.post(
            "/api/inspect", files={"file": ("j.xyz", fh, "application/octet-stream")}
        )
    assert r.status_code == 422
...
def test_request_body_size_cap(monkeypatch):
    ...
    r = client.post(
        "/api/certify/change", json={"measured_px": 5.0, "q_lo": 0.5, "q_hi": 0.5}
    )
...
def test_scan_upload_uses_upload_cap_not_json_body_cap(tmp_path, monkeypatch):
    ...
    q = tmp_path / "q.stl"
    q.write_bytes(b"not a mesh but larger than json cap")
    g = tmp_path / "g.stl"
    g.write_bytes(b"not a mesh but larger than json cap")
    r = client.post(
        "/api/identify/scan",
        files=[
...
def test_identify_scan_matches_same_arch(tmp_path):
    ...
    q = tmp_path / "q.stl"
    trimesh.creation.icosphere(subdivisions=5, radius=10).export(
        str(q)
    )  # >4000 verts -> exercises downsample
    g = tmp_path / "g.stl"
    trimesh.creation.icosphere(subdivisions=3, radius=10).export(str(g))
    o = tmp_path / "o.stl"
    trimesh.creation.box(extents=[20, 8, 6]).export(str(o))
    files = [
        ("files", ("q.stl", open(q, "rb"), "application/octet-stream")),
        ("files", ("g.stl", open(g, "rb"), "application/octet-stream")),
...
def test_identify_scan_needs_two(tmp_path):
    ...
    q = tmp_path / "q.stl"
    trimesh.creation.box().export(str(q))
    r = client.post(
        "/api/identify/scan",
...
def test_identify_scan_rejects_bad_file(tmp_path):
    ...
    bad.write_bytes(b"not a real mesh")
    r = client.post(
        "/api/identify/scan",
        files=[
            ("files", ("q.stl", open(q, "rb"), "application/octet-stream")),
...
def test_identify_scan_upload_cap(tmp_path, monkeypatch):
    ...
    trimesh.creation.box().export(str(q))
    g = tmp_path / "g.stl"
    trimesh.creation.box().export(str(g))
    r = client.post(
        "/api/identify/scan",
        files=[
            ("files", ("q.stl", open(q, "rb"), "application/octet-stream")),
