...
def surface_displacement(
    ...
    reconstruction noise into a positive bias ~k*sigma that inflates the
    certificate's conformal radius and hides small changes. Because
    ``E[mean ||v||^2] = (true displacement power) + (noise power)``, this estimates
    the magnitude in noise-power-corrected (squared) space::

        sqrt(max(0, mean ||v||^2 - noise_floor_sq))

    With ``noise_floor_sq`` = :func:`noise_floor_sq` of the *stable* calibration
    pairs, a no-change pair returns ~0 and a real displacement of magnitude m
    returns ~m, largely independent of the noise level. Caveat: the residual spread
    only shrinks with the point count for spatially *incoherent* noise; correlated
    reconstruction error does not average out, so the de-biasing gain is an upper
    bound.
    """
...
def assign_regions(
    ...
    Localises surface change: a real lesion moves a *patch*, which a whole-surface
    average dilutes; per-region measurement recovers the undiluted signal and says
    *where* it is. Returns an ``(N,)`` int array of region ids in ``[0, n_regions)``.
    """
...
def regional_displacements(
    ...
    false-change rate <= alpha despite the maximisation.
    """
    a = np.asarray(cloud_t0, float)
    b = np.asarray(cloud_t1, float)
    if a.shape != b.shape:
