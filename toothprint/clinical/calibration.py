...
class SiteCalibration:
    ...
    @classmethod
    def fit(cls, measured_stable, true_stable, *, site_id: str, created_utc: str,
            alpha: float = 0.1, min_calibration: int = 100) -> "SiteCalibration":
        ...
        cert = ConformalCertifier.fit(measured, np.asarray(true_stable, dtype=np.float64), alpha=alpha)
        return cls(certifier=cert, site_id=site_id, n_calibration=int(measured.size),
                   alpha=alpha, data_sha256=data_fingerprint(measured), created_utc=created_utc)
    ...
    def to_dict(self) -> dict:
        return {"site_id": self.site_id, "n_calibration": self.n_calibration,
                "alpha": self.alpha, "data_sha256": self.data_sha256,
                "created_utc": self.created_utc,
                "q_lo": self.certifier.q_lo, "q_hi": self.certifier.q_hi}

    @classmethod
    def from_dict(cls, d: dict) -> "SiteCalibration":
        return cls(certifier=ConformalCertifier(q_lo=d["q_lo"], q_hi=d["q_hi"], alpha=d["alpha"]),
                   site_id=d["site_id"], n_calibration=d["n_calibration"], alpha=d["alpha"],
                   data_sha256=d["data_sha256"], created_utc=d["created_utc"])
