...
from ._limits import (CorruptFile, FileTooLarge, MAX_IMAGE_PIXELS, UnsupportedFormat,
                      guard_pixels)
    ...
    for tag in ("ImagerPixelSpacing", "PixelSpacing"):
        ...
        if v is not None:
            try:
                ...
                pass
    return Radiograph(pixels=arr, pixel_spacing_mm=spacing, source_format="dicom",
...
def _load_raster(path: Path, fmt: str) -> Radiograph:
    if fmt == "tiff":
        ...
        try:
            with Image.open(path) as im:
                im.load()
                arr = np.asarray(im)
        except Image.DecompressionBombError as e:        # pragma: no cover - PIL's own bomb guard (guard_pixels is primary)
    ...
    return Radiograph(pixels=arr.astype(np.float32), pixel_spacing_mm=None,
                      source_format=fmt, bit_depth=int(arr.dtype.itemsize * 8), meta={})
